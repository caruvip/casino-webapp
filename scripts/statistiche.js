/**
 * statistiche.js — Dashboard statistiche utente con Chart.js
 * 
 * Popola la pagina user.html con:
 *  - Dati profilo (nome, saldo, VIP)
 *  - Statistiche aggregate (partite, win rate, ecc.)
 *  - Grafico a torta vittorie/sconfitte
 *  - Grafico lineare andamento saldo
 *  - Tabella cronologia ultime 20 partite
 *  - Tabella ultime 10 transazioni (ricariche)
 */
...
function getUser() {
    try { return JSON.parse(localStorage.getItem('casino_current_user')); }
    catch { return null; }
}
...
function populateStats(user) {
    ...
    set('netProfit',   formatCurrency(s.totalWon - s.totalLost));
    ...
    new Chart(canvas, {
        ...
        options: {
            ...
            plugins: {
                legend: {
                    position: 'bottom',
                    labels: { color: '#e0e0e0', font: { size: 13 }, padding: 16 }
                },
                tooltip: {
                    callbacks: {
                        label: ctx => ` ${ctx.label}: ${ctx.raw} (${s.gamesPlayed ? Math.round(ctx.raw / s.gamesPlayed * 100) : 0}%)`
                    }
                }
            }
        }
...
function buildBalanceChart(user) {
    ...
    const history = (user.history || []).slice().reverse(); // Cronologico: più vecchio → più recente
    if (history.length < 2) {
        // Nessuna storia sufficiente
    ...
    const balances = history.map(h => h.balance ?? 0);
...
// ─── Grafico a barre: vincite per gioco ───────────────────────────────────────
function buildGameBreakdownChart(user) {
    ...
    history.forEach(h => {
        ...
        if (!games[g]) games[g] = { won: 0, lost: 0, played: 0 };
        games[g].played++;
        if ((h.profit || 0) > 0) games[g].won += h.profit;
        else games[g].lost += Math.abs(h.profit || 0);
    });

    const labels = Object.keys(games).map(g => g.charAt(0).toUpperCase() + g.slice(1));
    const wonData  = Object.values(games).map(g => Math.round(g.won  * 100) / 100);
    const lostData = Object.values(games).map(g => Math.round(g.lost * 100) / 100);
    ...
    new Chart(canvas, {
        ...
        data: {
            labels,
            datasets: [
                { label: 'Vinto (€)',  data: wonData,  backgroundColor: 'rgba(34,197,94,0.7)'  },
                { label: 'Perso (€)',  data: lostData, backgroundColor: 'rgba(239,68,68,0.7)' },
            ]
        },
        options: {
            responsive: true,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    if (!user) {
        ...
        const loginPath = location.pathname.includes('giochi') ? '../login.html' : 'login.html';
    ...
    // I grafici richiedono Chart.js — attendiamo che sia caricato
    if (typeof Chart !== 'undefined') {
        buildWinLossChart(user);
        buildBalanceChart(user);
        buildGameBreakdownChart(user);
    } else {
        // Chart.js non ancora caricato: riprova dopo 500ms
    ...
    // Aggiornamento in tempo reale se il saldo cambia (es. aperta in un'altra tab)
    window.addEventListener('storage', (e) => {
        if (e.key === 'casino_current_user') {
            const updated = getUser();
            if (updated) {
                populateProfile(updated);
                populateStats(updated);
            }
        }
    });
